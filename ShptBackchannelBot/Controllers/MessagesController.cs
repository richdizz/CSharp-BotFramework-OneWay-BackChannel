    ...
    public class MessagesController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Post([FromBody]Activity activity)
        {
            ...
            else if (activity.Type == ActivityTypes.Message)
            {
                ...
                // Try to get username from BotState
                var state = activity.GetStateClient();
                var userdata = state.BotState.GetUserData(activity.ChannelId, activity.From.Id);
                var username = userdata.GetProperty<string>("username");
                if (!string.IsNullOrEmpty(username))
                {
                    // know the user
                    Activity reply = activity.CreateReply($"Sorry, I don't do much but the back channel told me you are {username}");
                    await connector.Conversations.ReplyToActivityAsync(reply);
                ...
                {
                    // don't know user yet
                    Activity reply = activity.CreateReply($"Sorry, I don't do much and I don't even know who you are yet");
                    await connector.Conversations.ReplyToActivityAsync(reply);
                }
            }
            else
